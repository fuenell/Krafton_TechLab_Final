...
void UMaterialInstanceDynamic::Serialize(const bool bInIsLoading, JSON& InOutHandle)
{
	...
	else // 저장
	{
		...
		const TMap<EMaterialTextureSlot, UTexture*>& OverriddenTexturesRef = GetOverriddenTextures();
		for (const auto& Pair : OverriddenTexturesRef)
		{
...
const FMaterialInfo& UMaterialInstanceDynamic::GetMaterialInfo() const
{
	if (bIsCachedMaterialInfoDirty)
	{
		...
		for (const auto& Pair : OverriddenTextures)
		{
			...
			UTexture* OverriddenTexturePtr = Pair.second;
			...
			const FString TexturePath = OverriddenTexturePtr ? OverriddenTexturePtr->GetFilePath() : FString();
